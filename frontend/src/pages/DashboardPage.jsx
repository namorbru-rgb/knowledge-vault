...
const cards = [
  ...
  { key: 'photos', label: 'Photos', icon: '📷', to: '/photos', color: 'from-green-600 to-green-800' },
  { key: 'notes', label: 'Notes', icon: '📝', to: '/notes', color: 'from-orange-600 to-orange-800' },
]
...
export default function DashboardPage() {
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-slate-400 mb-8">Your knowledge at a glance</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {cards.map(card => (
            <Link key={card.key} to={card.to}
              ...
              <span>{card.icon}</span> Add {card.label.slice(0,-1)}
            </Link>
          ))}
        </div>
      </div>
