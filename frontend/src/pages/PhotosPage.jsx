...
export default function PhotosPage() {
  ...
  return (
    <div className="p-8">
      <h2 className="text-2xl font-bold text-fg mb-6">Fotos</h2>
      <div className="mb-8">
      ...
      {loading ? <p className="text-muted">Lädt...</p> : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {photos.length === 0 && <p className="text-muted col-span-full">Noch keine Fotos. Lade welche hoch!</p>}
          {photos.map(photo => (
            <div key={photo.id} className="bg-surface border border-line rounded-xl overflow-hidden group">
              ...
              <div className="p-3">
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <button onClick={() => deletePhoto(photo.id)} className="text-subtle hover:text-red-600 dark:text-red-400 transition-colors opacity-0 group-hover:opacity-100">🗑️</button>
                </div>
              </div>
            </div>
          ))}
