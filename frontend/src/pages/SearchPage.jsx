  ...
  return (
    <div className="p-8">
      ...
      <form onSubmit={doSearch} className="flex gap-3 mb-8">
        <input type="text" placeholder="Search your knowledge..." value={query} onChange={e => setQuery(e.target.value)}
      ...
      {searched && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <p className="text-slate-400 text-sm">{results.length} results {mode && <span className="text-slate-600">({mode} search)</span>}</p>
          </div>
          {results.length === 0 && !loading && <p className="text-slate-400">No results found for "{query}"</p>}
                ...
                <div key={i} className="bg-slate-800 border border-slate-700 rounded-xl p-4 hover:border-slate-600 transition-colors">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-white truncate">{r.title || 'Untitled'}</p>
                      {r.content && <p className="text-sm text-slate-400 mt-1 line-clamp-2">{r.content}</p>}
                      {r.similarity && <p className="text-xs text-slate-600 mt-1">{Math.round(r.similarity * 100)}% match</p>}
                    </div>
                  </div>
                </div>
              )
      ...
      {!searched && (
        <div className="text-center py-16">
          ...
          <p className="text-slate-400">Search across all your videos, links, photos, and notes</p>
          <p className="text-slate-500 text-sm mt-2">Semantic AI search when pgvector is enabled</p>
        </div>
      )}
    </div>
  )
